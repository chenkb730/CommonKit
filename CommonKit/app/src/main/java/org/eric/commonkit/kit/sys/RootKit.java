...
import org.eric.commonkit.kit.MsgKit;
        ...
        new Thread() {
            public void run() {
                ...
                try {
                    ...
                    LogKit.i("root status =" + len);
                    if (-1 == len && callback != null)
                        MsgKit.sendMessage(callback, MsgKit.MSG_FAIL, null);
                    else if (callback != null)
                        MsgKit.sendMessage(callback, MsgKit.MSG_SUCCESS, null);
                } catch (Exception e) {
                } finally {
                    try {
            ...
            public void run() {
                ...
                try {
                    for (String path : paths) {
                        ...
                        process = Runtime.getRuntime().exec("su");
                        if (callback != null)
                            MsgKit.sendMessage(callback, MsgKit.MSG_START, msgobj);
                        out = process.getOutputStream();
                        // 调用安装
                        out.write(("pm install -r " + path + "\n").getBytes());
                        in = process.getInputStream();
    ...
    public static void uninstall(final Context context, final List<String> packageNames, final Object msgobj,
        ...
        new Thread() {
            public void run() {
                ...
                InputStream in = null;
                try {
                    for (String pkg : packageNames) {// 请求root
                        process = Runtime.getRuntime().exec("su");
                        if (callback != null)
                            MsgKit.sendMessage(callback, MsgKit.MSG_START, msgobj);
                        out = process.getOutputStream();
