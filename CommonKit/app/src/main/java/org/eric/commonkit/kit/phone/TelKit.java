...
import org.eric.commonkit.kit.PrefKit;
...
public class TelKit {
    ...
    public static int getCallState() {
        TelephonyManager tm = ManageKit
                .getTelephonyManager(CKApplication.context);
    ...
    public static String getMacAddress() {
        WifiInfo info = ManageKit.getWifiManger(CKApplication.context)
                .getConnectionInfo();
    ...
    public static String getUUID() {
        final TelephonyManager tm = ManageKit
                .getTelephonyManager(CKApplication.context);
        final String androidId = ""
                + android.provider.Settings.Secure.getString(
                CKApplication.context.getContentResolver(),
                android.provider.Settings.Secure.ANDROID_ID);

        return new UUID(androidId.hashCode(),
                ((long) ("" + tm.getDeviceId()).hashCode() << 32)).toString();
    ...
    public static String getId() {
        ...
        if (uid == null)
            uid = TelKit.getSubscriberId();
        if (uid == null)
            uid = TelKit.getLine1Number();

        if (uid != null) {
            final String ABASE_DEVICE_ID = "ABASE_DEVICE_ID";
            String old = PrefKit.getString(ABASE_DEVICE_ID, null);
            if (!ValidateKit.equals(old, uid))
                PrefKit.setValue(ABASE_DEVICE_ID, uid);
        } else
            uid = PrefKit.getString("ABASE_DEVICE_ID", null);
