...
import org.eric.commonkit.db.sqlite.CursorUtils;
import org.eric.commonkit.db.sqlite.DbModel;
import org.eric.commonkit.db.sqlite.ManyToOneLazyLoader;
import org.eric.commonkit.db.sqlite.OneToManyLazyLoader;
import org.eric.commonkit.db.sqlite.SqlBuilder;
import org.eric.commonkit.db.sqlite.SqlInfo;
import org.eric.commonkit.db.table.KeyValue;
import org.eric.commonkit.db.table.ManyToOne;
import org.eric.commonkit.db.table.OneToMany;
import org.eric.commonkit.db.table.TableInfo;
import org.eric.commonkit.exception.DbException;
...
public class CkDb {
    private final String TAG = "ckdb";
    private static HashMap<String, CkDb> daoMap = new HashMap<String, CkDb>();

    private SQLiteDatabase db;
    private DaoConfig config;

    private CkDb(DaoConfig config) {
        if (config == null)
    ...
    private synchronized static CkDb getInstance(DaoConfig daoConfig) {
        CkDb dao = daoMap.get(daoConfig.getDbName());
        if (dao == null) {
            dao = new CkDb(daoConfig);
     ...
     * @param context
     */
    public static CkDb create(Context context) {
     ...
     */
    public static CkDb create(Context context, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     * @param dbName  数据库名称
     */
    public static CkDb create(Context context, String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     */
    public static CkDb create(Context context, String dbName, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
        config.setDebug(isDebug);
     ...
     * @param dbName  数据库名称
     */
    public static CkDb create(Context context, String targetDirectory,
                              String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
     ...
     * @return
     */
    public static CkDb create(DaoConfig daoConfig) {
