package org.eric.commonkit.kit.network;
...
import org.eric.commonkit.kit.common.Provider;
import org.eric.commonkit.kit.common.ValidateKit;
import org.eric.commonkit.kit.phone.TelKit;


public class ApnKit {

    /**
     * 因为 wap 访问不了 www 所以 要改一下 接入点 必需 为 系统应用 或者有系统权限 暂不支持 中国电信
     */
    public static void changeWAPToNet() {
        int operator = TelKit.getSimOperator();

        LogKit.i("operator =  " + operator + " net work type ="
                + NetworkKit.getNetworkType());
        LogKit.i(" now apn is =" + ApnKit.isWapApn());
        ...
        switch (NetworkKit.getNetworkType()) {
            case TelephonyManager.NETWORK_TYPE_GPRS: // mobile and unicom 2g gprs
                ...
                if (operator == TelKit.CHINA_MOBILE) {
                    LogKit.i("china mobile ");
                    ApnKit.setCmNetApn();

                } else if (operator == TelKit.CHINA_UNICOM) {
                    LogKit.i("china unicom");
                    ApnKit.setUniNet();
                }
                break;
    ...
    public static boolean isWapApn() {
        ...
        ContentResolver contentResolver = CKApplication.context
                .getContentResolver();
        Cursor cursor = contentResolver.query(Provider.PREFERRED_APN_URI, null,
                null, null, null);
        if (cursor != null) {
    ...
    private static int findAPNId(ContentValues cv) {
        ...
        ContentResolver contentResolver = CKApplication.context
                .getContentResolver();
        Cursor cursor = contentResolver.query(Provider.ALL_APN_URI, null, null,
                null, null);
        if (cursor != null) {

            while (cursor.moveToNext()) {
                String apn = cursor.getString(cursor.getColumnIndex("apn"));
                String numeric = cursor.getString(cursor
                        .getColumnIndex("numeric"));

                if (ValidateKit.equals(cv.getAsString("apn"), apn)
                        && ValidateKit.equals(cv.getAsString("numeric"),
                        numeric)) {
